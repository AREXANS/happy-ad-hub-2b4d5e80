...
const PaymentSuccess: FC<PaymentSuccessProps> = ({ finalData, onCopy }) => {
  const [scriptText, setScriptText] = useState('Loading...');
  const [socialLinks, setSocialLinks] = useState<SocialLink[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      // Fetch loadstring from correct key
      ...
      .on(
        ...
        (payload) => {
          if (payload.new && (payload.new as { value: string }).value) {
            setScriptText((payload.new as { value: string }).value);
          }
        }
      )
      .subscribe();
